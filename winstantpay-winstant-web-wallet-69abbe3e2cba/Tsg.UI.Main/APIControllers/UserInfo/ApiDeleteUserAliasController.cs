...
{
    [ApiFilter]
    public class ApiDeleteUserAliasController : ApiController
    {
        /// <summary>
        /// Delete a user alias
        /// </summary>
        ...
        /// <returns></returns>
        [HttpPost]
        //[Route("api/DeleteUserAlias")]        
        public IHttpActionResult Post([FromBody] ApiDeleteUserAliasRequest request)
        {
            ...
            try
            {
                ...
                if (isUserTokenContains && userRepository.IsVerificatedUser(outerUserToken.First(), out ui))
                {
                    ...
                    result.Success = userInfoMethods.DeleteAlias(request.Alias, out var res);
                    if(result.Success)
