...
namespace Tsg.UI.Main.Models
{
	public class NewInstantPaymentViewModel : BaseViewModel
	{
		...
		/// </summary>
		/// <returns></returns>
		public IList<CurrencyData> GetFXBuyCurrencies()
		{
			var currencies = new List<CurrencyData>();
			var currenciesResponse = Service.GetFXBuyCurrencies();
			if (!currenciesResponse.ServiceResponse.HasErrors)
			{
				currencies = currenciesResponse.Currencies.ToList();
			}
			...
			var allowedCurrencies = AppSettingHelper.GetAllowedCurrencies();
			_logger.DebugFormat("Allowed currencies, count: {0} items: {1}", allowedCurrencies.Count, JsonConvert.SerializeObject(allowedCurrencies));
			if (allowedCurrencies.Count > 0 && !string.IsNullOrEmpty(allowedCurrencies[0]))
			{
				currencies = currencies.Where(c => allowedCurrencies.Any(ac => ac == c.CurrencyCode)).ToList();
				_logger.DebugFormat("after filter allowed currencies, currencies: {0}", JsonConvert.SerializeObject(currencies));
			}
		...
		/// </summary>
		/// <returns></returns>
		public IList<CurrencyData> GetFXSellCurrencies()
		{
			var currencies = new List<CurrencyData>();
			var currenciesResponse = Service.GetFXSellCurrencies();
			if (!currenciesResponse.ServiceResponse.HasErrors)
			{
				currencies = currenciesResponse.Currencies.ToList();
			}
