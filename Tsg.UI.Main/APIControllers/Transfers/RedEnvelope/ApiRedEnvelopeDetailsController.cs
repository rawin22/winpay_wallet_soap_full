...
{
    public class ApiRedEnvelopeDetailsController : ApiController
    {
        ...
        public ApiRedEnvelopeDetailsController(IDependencyLiquidForUserServiceMethods dependencyLiquidForUserService,
            ...
            try
            {
                ...
                if (isUserTokenContains && userRepository.IsVerificatedUser(outerUserToken.First(), out var ui))
                {
                    ...
                    if (!transfer.Success || transfer.Obj == null || transfer.Obj.Transfers_SourceType != TransfersSourceTypeEnum.RedEnvelope)
                        return Content(HttpStatusCode.BadRequest, new StandartResponse("Error by red envelope get details", transfer.Message));
                    ...
                    if (!redEnvelope.Success || redEnvelope.Obj == null)
                        return Content(HttpStatusCode.BadRequest, new StandartResponse("Error by red envelope get details", redEnvelope.Message));
                    return Content(HttpStatusCode.OK, new StandartResponse<RedEnvelopeSo>(redEnvelope.Obj, redEnvelope.Success, redEnvelope.Message));
                }
                else
                {
                    return Unauthorized();
