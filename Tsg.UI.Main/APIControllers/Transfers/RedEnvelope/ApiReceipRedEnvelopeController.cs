...
{
    [ApiFilter]
    public class ApiReceipRedEnvelopeController : ApiController
    {
        ...
        public ApiReceipRedEnvelopeController(IDependencyLiquidForUserServiceMethods dependencyLiquidForUserService,
            ...
            try
            {
                ...
                if (isUserTokenContains && userRepository.IsVerificatedUser(outerUserToken.First(), out var ui))
                {
                    ...
                    if (!resTransferedObj.Success || resTransferedObj.Obj == null || resTransferedObj.Obj.Transfers_SourceType != TransfersSourceTypeEnum.RedEnvelope)
                        return Content(HttpStatusCode.BadRequest, new StandartResponse("Not found transfered  by this Id."));
                    ...
                    if (resTransferedObj.Obj.Transfers_AcceptedDate.HasValue && (!resTransferedObj.Obj.Transfers_IsRejected.HasValue || !(bool)resTransferedObj.Obj.Transfers_IsRejected))
                        return Content(HttpStatusCode.BadRequest, new StandartResponse("Transfered red envelope was activated early"));
                    
                    if (resTransferedObj.Obj.Transfers_IsRejected.HasValue && (bool)resTransferedObj.Obj.Transfers_IsRejected)
                        return Content(HttpStatusCode.BadRequest, new StandartResponse("Transfered red envelope was rejected early"));
                    ...
                    if (!userRecipient.Success || userRecipient.Obj == null)
                        return Content(HttpStatusCode.BadRequest, new StandartResponse("User not found for this transfered red envelope by this Id."));
                    ...
                    if (resTransferedObj.Obj.Transfers_LinkToSourceRow.HasValue)
                    {
                        ...
                        if (!redEnvelope.Success || redEnvelope.Obj == null)
                            return Content(HttpStatusCode.BadRequest, new StandartResponse("Error by red envelope transfer", redEnvelope.Message));
                        ...
                        if (res.ServiceResponse.HasErrors)
                            return Content(HttpStatusCode.BadRequest, new StandartResponse(StringExtensions.ConvertServiceResponseToSingleString(res.ServiceResponse.Responses)));
                        ...
                        if (paymentTorecipientResult.ServiceResponse.HasErrors)
                            return Content(HttpStatusCode.BadRequest, new StandartResponse(StringExtensions.ConvertServiceResponseToSingleString(paymentTorecipientResult.ServiceResponse.Responses)));
                        ...
                        redEnvelope.Obj.RedEnvelope_RecipientPaymentId = model.Action ? Guid.Parse(ui.UserId) : (Guid?) null;
                        redEnvelope.Obj.RedEnvelope_RecipientUserName = ui.UserName;
                    ...
                    if (!trasferRes.Success)
                        return Content(HttpStatusCode.BadRequest, new StandartResponse("Error by red envelope transfer", trasferRes.Message));
                    return Ok(new StandartResponse<RedEnvelopeSo>(model.Action ? redenvelopeGlobal?.Obj : (RedEnvelopeSo) null,  true, model.Action ? "Transfer for red envelope finished successfully": "Transfer for red envelope is rejected"));
                }
                else
                {
                    return Unauthorized();
