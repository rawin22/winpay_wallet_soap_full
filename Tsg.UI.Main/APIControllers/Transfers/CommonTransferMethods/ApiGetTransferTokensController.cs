...
{
    [ApiFilter]
    public class ApiGetTransferTokensController : ApiController
    {
        ...
        public ApiGetTransferTokensController(ITransfersServiceMethods transfersServiceMethods, IDaPayLimitsServiceMethods daPayLimitsService)
        {
            ...
            try
            {
                ...
                if (isUserTokenContains && userRepository.IsVerificatedUser(outerUserToken.First(), out var ui))
                {
                    var transfers = _transfersServiceMethods.GetAll().Obj.Where(w => w.Transfers_SourceType == TransfersSourceTypeEnum.TokenTransfers &&
                        w.Transfers_TransferRecipient.ToLower() == ui.UserName.ToLower() && (!w.Transfers_IsRejected.HasValue || w.Transfers_IsRejected.Value==false)
                    ...
                    return Ok(new StandartResponse<List<TransfersTokenSo>>(transfersToken, true,
                        transfersToken.Count > 0 ? "Ok" : "Empty enumerable"));
                }
                else
                {
                    return Unauthorized();
