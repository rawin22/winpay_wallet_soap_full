...
namespace Tsg.UI.Main.APIControllers.UserInfo
{
    ...
    public class ApiUserProfileController : ApiController
    {
        ...
        public ApiUserProfileController(IDaPayLimitsTabServiceMethods daPayLimitsTabServiceMethods, IDaPayLimitsTypeServiceMethods daPayLimitsTypeServiceMethods, IDaUserWPayIDSettingServiceMethods daUserWPayIDSettingServiceMethods)
        {
        ...
        [HttpPost]
        public IHttpActionResult Post([FromBody] ViewProfileModel model)
        {
            UserRepository userRepository = new UserRepository();
            try
            {
                ...
                if (isUserTokenContains && userRepository.IsVerificatedUser(outerUserToken.First(), out var ui))
                {
                    ...
                    model.InfoBlock = new InfoBlock() { Code = ApiErrors.ErrorCodeState.Success, UserMessage = string.Empty, DeveloperMessage = "Ok" };
                    model.Success = true;
        ...
        private void SaveDaUserWPayIdSetting(List<DaUserWPayIDSettingSo> settings)
        {
            foreach (var setting in settings)
            {
                ...
                {                    
                    ...
                    {
                        daUserSettingSo.IsPinRequired = setting.IsPinRequired;
                        daUserSettingSo.PinCode = setting.PinCode;
                        daUserSettingSo.ExceedingAmount = setting.ExceedingAmount;
        ...
        [NonAction]
        private void SaveDaUserWPayIdSetting(ViewProfileModel model)
        {
            foreach (var setting in model.DaUserWPayIDSettings)
            {
                if (setting.ID == default)
                {
                    model.InfoBlock = new InfoBlock() { Code = ApiErrors.ErrorCodeState.Success, UserMessage = "No ID-Insert", DeveloperMessage = "SaveDaUserWPayIdSetting-ID is default-Before Insert" };
                    setting.ID = Guid.NewGuid();

                    setting.IsDeleted = false;
                    var resInsert = _daUserWPayIDSettingServiceMethods.Insert(setting);
                    model.InfoBlock = new InfoBlock() { Code = ApiErrors.ErrorCodeState.Success, UserMessage = "", DeveloperMessage = "SaveDaUserWPayIdSetting-After Insert. Message: " +resInsert.Message };
                }
                else
                {
                    model.InfoBlock = new InfoBlock() { Code = ApiErrors.ErrorCodeState.Success, UserMessage = "ID Available-Update", DeveloperMessage = "SaveDaUserWPayIdSetting-ID is available-Update" };
                    var daUserSettingSo = _daUserWPayIDSettingServiceMethods.GetById(setting.ID).Obj;
                    if (daUserSettingSo != null)
                    {
                        model.InfoBlock = new InfoBlock() { Code = ApiErrors.ErrorCodeState.Success, UserMessage = "daUserSettingSo is not null", DeveloperMessage = "SaveDaUserWPayIdSetting-daUserSettingSo is not null-Before Update" };
                        daUserSettingSo.IsPinRequired = setting.IsPinRequired;
                        daUserSettingSo.PinCode = setting.PinCode;
                        daUserSettingSo.ExceedingAmount = setting.ExceedingAmount;

                        var recUpdate = _daUserWPayIDSettingServiceMethods.Update(setting);
                        model.InfoBlock = new InfoBlock() { Code = ApiErrors.ErrorCodeState.Success, UserMessage = "", DeveloperMessage = "SaveDaUserWPayIdSetting-After Update. Message: " + recUpdate.Message };
                    }
                }
            }
        }
