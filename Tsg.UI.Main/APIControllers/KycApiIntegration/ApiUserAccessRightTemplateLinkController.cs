...
{
    public class ApiUserAccessRightTemplateLinkController : ApiController
    {
        readonly log4net.ILog _logger = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        [HttpPost]
        public IHttpActionResult Post([FromBody] UserAccessRightTemplateLinkModel model)
        {
            _logger.Info("[POST] ApiUserAccessRightTemplateLinkController start");

            var result = new UserAccessRightTemplateLinkResponseModel
            {
                Success = false,
            ...
            try
            {
                ...
                if (isUserTokenContains && userRepository.IsVerificatedUser(outerUserToken.First(), out ui))
                {
                    ...
                    result = method.UserAccessRightTemplateLink(model);
                }
