...
{
    public class ApiCustomerUserCreateController : ApiController
    {
        readonly log4net.ILog _logger = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        [HttpPost]
        public IHttpActionResult Post([FromBody] CustomerUserCreateModel model)
        {
            _logger.Info("[POST] ApiCustomerUserCreateController start");
            var result = new CustomerUserCreateResponseModel
            {
                Success = false,
            ...
            try
            {
                if(!ModelState.IsValid)
                    throw new Exception("Model is not correct, fill all required proreties");
                ...
                if (isUserTokenContains && userRepository.IsVerificatedUser(outerUserToken.First(), out ui))
                {
                    ...
                    result = method.CustomerUserCreate(model);
                    if (!result.Success)
                        return Content(HttpStatusCode.BadRequest, result as StandartResponse);
                }
