...
namespace Tsg.UI.Main.APIControllers
{
    ...
    /// </summary>
    [ApiFilter]
    public class ApiFXDealQuoteController : ApiController
    {
        /// <summary>
        ...
        [HttpPost]
        public IHttpActionResult Post([FromBody]ApiFxQuoteCreateRequest model)
        {
            var result = new ApiFxQuoteCreateResponse()
            {
                Success = false,
            ...
            try
            {
                ...
                {
                    result.InfoBlock = new InfoBlock() { Code = ApiErrors.ErrorCodeState.EmptyResult, DeveloperMessage = "InfoBlock. WindowOpenDate or FinalValueDate is invalid", UserMessage = "InfoBlock. Value is null" };
                    return Ok(result);
                }

                FxQuoteMethods quoteModel = new FxQuoteMethods(ui);
                var createResponse =  quoteModel.Create(model);

                if (createResponse.Quote != null && !String.IsNullOrEmpty(createResponse.Quote.QuoteId))
                {
                    result.Quote = createResponse.Quote;
                    result.Success = true;
                    result.InfoBlock = new InfoBlock() { Code = ApiErrors.ErrorCodeState.Success, DeveloperMessage = "OK. FX Quote created successfully", UserMessage = "OK. FX Quote created successfully" };
                }
                else
                {

                    result.Errors = createResponse.Errors;
                    result.InfoBlock = new InfoBlock() { Code = ApiErrors.ErrorCodeState.UnspecifiedError, DeveloperMessage = "FX quotation creation is failed", UserMessage = "FX quotation creation is failed" };
                }
            }
            catch (Exception e)
            {
