                    ...
                    if (!checkOrderIfExist.IsExist)
                    {
                        try
                        {
                            var tokenGuid = ApiLogic.GetNewOrderKey(
                                ...
                                model.UrlReturn, model.Custom, model.IsSandbox);
            ...
            try
            {
                ...
                if (IsParsedData(order, resCheckToken, ref model, ref checker, ref errorsList))
                {
                    try
                    {
                        ...
                        if (!prepareOrder.IsSuccess)
                            throw new Exception(prepareOrder.Message);
                        if (String.IsNullOrEmpty(model.Alias) || model.Account == 0)
                        {
                            throw new Exception("Please select other account and alias");
                        }
                        ...
                        if (getInstantantForOrder.IsSuccesifull)
                        {
                            ...
                            else throw new Exception(paymentOrderProcess.ResultAnswer.ResultText.Split(new []{"InnerDetail"}, StringSplitOptions.RemoveEmptyEntries)[1]);
                        }
                        else
                        {
